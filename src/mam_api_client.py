...
class MAMClient:
    ...
    def __init__(self, username: str = None, password: str = None, session_cookie: str = None):
        ...
        if self.username and self.password:
            ...
            self.session.headers.update({'Cookie': f'mam_id={session_cookie}'})
            self.logged_in = True
    
    def login(self) -> bool:
        """Login to MAM using username/password"""
        if not self.username or not self.password:
            self.logger.error("Username and password required for login")
            return False
        
        login_url = f"{self.base_url}/tor/takelogin.php"
        login_data = {
        ...
        try:
            ...
            self.session.get(f"{self.base_url}/tor/login.php")
            ...
            if response.status_code == 200 and 'login.php' not in response.url:
                self.logged_in = True
                self.logger.info("Successfully logged into MAM")
                return True
            else:
    ...
    def search_ebooks(self, audiobook: AudiobookMetadata) -> List[MAMSearchResult]:
        ...
        if not self.logged_in:
            self.logger.error("Not logged in to MAM")
            return []
...
def main():
    ...
    if not USERNAME or not PASSWORD:
        if not SESSION_COOKIE:
            print("Error: Either MAM_USERNAME/MAM_PASSWORD or MAM_SESSION_COOKIE must be set")
            return
    
    # Initialize components
    extractor = AudiobookExtractor(DB_PATH)
    
    # Try username/password first, fallback to session cookie
    if USERNAME and PASSWORD:
        print(f"Logging in with username: {USERNAME}")
        mam_client = MAMClient(username=USERNAME, password=PASSWORD)
    else:
        print("Using session cookie authentication")
        mam_client = MAMClient(session_cookie=SESSION_COOKIE)
