...
const API_BASE_URL =
  process.env.NEXT_PUBLIC_API_URL || "https://localhost:5563";
const API_KEY = process.env.NEXT_PUBLIC_API_KEY || "babel_secure_YOUR_KEY_HERE";
const HR_API_BASE_URL =
  process.env.NEXT_PUBLIC_HR_API_URL || "http://localhost:8081";
const QA_API_BASE_URL =
  process.env.NEXT_PUBLIC_QA_API_URL || "http://localhost:8082";
...
class LibraryOfBabelAPI {
  ...
  constructor(
    ...
    hrBaseURL: string = HR_API_BASE_URL,
    qaBaseURL: string = QA_API_BASE_URL,
  ) {
  ...
  // HR API Methods - Using Next.js API routes for CORS handling
  private async makeHRRequest<T>(
    endpoint: string,
    options: RequestInit = {},
  ): Promise<T> {
    // Convert HR agent endpoints to Next.js API routes
    const nextApiEndpoint = endpoint.replace('/hr/', '/api/hr/');
    const baseUrl = typeof window !== 'undefined' ? window.location.origin : '';
    const url = `${baseUrl}${nextApiEndpoint}`;
  ...
  async getHRAgents(): Promise<{ agents: HRAgent[] }> {
    try {
      ...
      console.warn(
        "HR agents endpoint unavailable, returning mock data:",
        error,
      );
      return {
        agents: [
  ...
  async getHRAlerts(): Promise<{ alerts: HRAlert[] }> {
    try {
      ...
      // Return mock data when database is unavailable
      console.warn(
        "HR alerts endpoint unavailable, returning mock data:",
        error,
      );
      return {
        alerts: [
  ...
  // QA API Methods - Using Next.js API routes for CORS handling
  private async makeQARequest<T>(
    endpoint: string,
    options: RequestInit = {},
  ): Promise<T> {
    // Convert QA agent endpoints to Next.js API routes
    const nextApiEndpoint = endpoint.replace('/hr/qa/', '/api/qa/');
    const baseUrl = typeof window !== 'undefined' ? window.location.origin : '';
    const url = `${baseUrl}${nextApiEndpoint}`;
   ...
   */
  async getQAReports(): Promise<Record<string, unknown>> {
   ...
   */
  async getQATraining(): Promise<Record<string, unknown>> {
   ...
   */
  async getQAMentorship(): Promise<Record<string, unknown>> {
   ...
   */
  async getQATestResults(): Promise<Record<string, unknown>> {
